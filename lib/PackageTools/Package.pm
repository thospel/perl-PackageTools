...
# $Id: Package.pm 4192 2010-09-15 15:31:10Z hospelt $

# START HISTORY
# autogenerated by release_pm
use vars qw($VERSION $release_time %history);
$VERSION = "1.007";
$release_time = 1284556430;
%history = (
  'Changes' => {
    '1.000' => '1.000',
    '1.001' => '1.001',
...
sub release_time {
    if (!defined $epoch_base) {
        require Time::Local;
        $epoch_base = Time::Local::timegm(0,0,0,1,0,70);
    }
...
PackageTools::Package.pm - Version and history of PackageTools
...
L<release_pm(1)|release_pm> was run (even if that was on another system with a
different epoch). Since the idea is to run L<release_pm(1)|release_pm> just
...
L<release_pm(1)> which can be used to keep Package.pm files up to date.
